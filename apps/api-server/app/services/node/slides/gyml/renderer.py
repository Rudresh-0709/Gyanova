...
{animator_js}
    ...
    def _render_cyclic_block(self, node: "GyMLCyclicBlock") -> str:
        ...
        # SVG geometry config
        cx, cy = 250, 250  # centre of SVG canvas
        R_outer = 180  # outer radius of the ring
        R_inner = 75  # inner radius (hole)
        R_icon = (R_outer + R_inner) / 2  # icon sits at mid-ring
        gap_deg = 8  # gap between sectors (degrees)
        viewbox_size = 500
        ...
        def chevron_arc_path(cx, cy, R_out, R_in, start_deg, sweep, chevron_deg=8.0):
            """
            Donut-sector with chevron (V-shape) interlocking edges.
            The 'end' edge has a protruding tip at R_mid.
            The 'start' edge has a matching notch.
            This creates the 'arrow' effect.
            """
            R_mid = (R_out + R_in) / 2
            ...
            d = f"M {ox1:.3f},{oy1:.3f}"
            d += f" A {R_out},{R_out} 0 {large},1 {ox2:.3f},{oy2:.3f}"  # Outer arc CW
            d += f" L {tx:.3f},{ty:.3f}"  # To tip
            d += f" L {ix1:.3f},{iy1:.3f}"  # To inner end
            d += f" A {R_in},{R_in} 0 {large},0 {ix2:.3f},{iy2:.3f}"  # Inner arc CCW
            d += f" L {nx:.3f},{ny:.3f}"  # To notch
            d += " Z"
            return d
        ...
        # Chevron angle: the protrusion depth of the arrow tip
        chevron_deg = 10.0
        ...
        for i, item in enumerate(node.items):
            ...
            # Icon anchor (foreignObject for i-tag)
            if item.icon:
                icon_class = item.icon
                if not icon_class.startswith("ri-"):
                    icon_class = f"ri-{icon_class}-line"
...
.p-annotation {
    font-size: 1.15rem;
    font-size: calc(var(--p-size, 1.1rem) * 1.1);
